		...
		return getSenderKeyMutex(senderKeyName).runExclusive(async () => {
			...
			for (const queuedMessage of queue) {
				try {
					...
					logger.warn({ senderKeyName, error: (error as Error).message }, 'queued message still failed after sender key available')
				...
				if (hasSenderKeys) {
					...
					for (const senderKeyName of senderKeyNames) {
						await queueSenderKeyOperation(senderKeyName, async () => {
							...
							const senderKeyValue = data['sender-key']![senderKeyName]
							const senderKeyData: SignalDataSet = {
								'sender-key': {
									[senderKeyName]: senderKeyValue ?? null
								}
							}
